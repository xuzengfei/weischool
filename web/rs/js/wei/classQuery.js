...
function  listCourse(rdata) {
    $.getJSON(basePath+"wei/course/class/register/datagrid?pageNo="+pageNo+"&gradId="+moduleId,rdata, function(res){
        if(res.success){
...
function loadTopData() {
    $.getJSON(basePath+"wei/course/rest/class/"+moduleId,function(res){
        if(res.success){
            var data =res.obj;

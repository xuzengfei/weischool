 ...
 */
function listInform() {
    $.getJSON(basePath + "wei/notice/datagrid?pageNo=" + pageNo, function (res) {
        if (res.success) {
            var data = res.obj.datas;//列表
            recordCount = res.obj.recordCount;
            $.each(data, function (i, item) {
                if (pageNo == 1 && i == 0) {
                    $(".headTitle").parent().attr("href",basePath + "wei/notice/to/msg/view/"+item.id);
                    ...
                    $(".list-group").append("<a href=\""+basePath + "wei/notice/to/msg/view/"+item.id+"\">" +

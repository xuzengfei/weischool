...
$(function () {
    $(".dropdown-menu").find("li").find("a").each(function () {
        $(this).click(function () {
            var area = $(this).text();
            $("#myTabDrop1").html(area + "<b class='caret'></b>");
        });
    });
});
...
function loadStPic(basePath) {
    var n = 0;
    $.getJSON(basePath + 'wei/attach/list/st/pic', function (response) {
        if (response.success) {
        ...
        if (n == 0) {
            $(".img-circle").attr("src", basePath + "rs/css/wei/images/index_userImg.png");
        }

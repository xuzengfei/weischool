...
<body>
<div class="pd-20">
    <form action="${ pageContext.request.contextPath }/web/manager/expe/${empty obj.id?"add":"edit" }" method="post"
          class="form form-horizontal" id="form-admin-add">
        <input type="hidden" name="id" value="${obj.id }">
        <input type="hidden" name="expeTime" value="${obj.expeTime }">
        <c:if test="${empty obj.id}">
            <div class="row cl">
                <label class="form-label col-3"><span class="c-red">*</span>学号：</label>
                <div class="formControls col-5">
                    <input type="text" class="input-text" placeholder="输入学号" readonly value="${empty obj.student.no?stNo:obj.student.no }" onclick="radioSSelect('stNo')"
                           name="student.no" id="stNo" datatype="*2-16" nullmsg="学号不能为空">
                </div>
                <div class="col-4"></div>
            </div>
        </c:if>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5">
                <input type="text" class="input-text wdate" placeholder="选择时间"
                       value='<t:date value="${obj.expeTime}"   pattern="yyyy-MM-dd"/>'
                       onclick="WdatePicker({onpicked:function(){
					$('input[name=expeTime]').val(strTotimestamp($(this).val()));
					   }})" datatype="*2-20" nullmsg="请选择时间">
            </div>
            <div class="col-4"></div>
        </div>
        <div class="row cl">
            <label class="form-label col-3">活动经历：</label>
            <div class="formControls col-5">
                    ...
                    <ul class="cl portfolio-area">
                        <li class="item uploadFile">
                            <div class="portfoliobox">
                                <div class="picbox"><a href="javascript:void(0)" id="uploadFile" data-lightbox="gallery"
                                                       data-title=" "><img
                                        src="${ pageContext.request.contextPath }/rs/images/icon-add.png"></a></div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-5 skin-minimal">
                <div class="radio-box">
                    <input type="radio" id="sex-1" name="isenable"  ${obj.isenable eq 1?"checked":""} value=1
                           datatype="*" nullmsg="请选择启用/禁用！">
...
<script type="text/javascript">
    ...
    var moduleId = "${obj.id}";
    var uploadcontroller = '${ pageContext.request.contextPath }/web/manager/attach/experience/${empty obj.id?-1:obj.id}/1/2';
    $(function () {
